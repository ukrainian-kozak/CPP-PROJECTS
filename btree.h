...
namespase btree {
    ...
    //Types small_ and big_ are promisw that sizeof(small_) < sizeof(big_)
    typedef char small_;
    ...
    struct btree_can_extract_key<_Pair, Key, std::pair<First, Second>>
        : std::conditional<std::is_same<typename std::remove_const<First>::type, Key>,
            btree_extract_key_first_tag,
            btree_extract_key_fail_tag>::type { };


    //btree_can_extra_map_key uses true_type/false_type instead of the tags.
    //It returns true if Key != ContainerValueType (the container is a map not a set)
    //and ValueType == Key.
    template <typename ValueType, template Key, template ContainerValueType,
             template RawValueType = typename std::remove_const<typename std::remove_reference<ValueType>::type>::type>
    ...
    //A helper type used to indicate that a key-compare-to functor has been provided.
    //A user can specify a key-compare functor by doing:
    //
    //struct MyCompare 
    ...
    //A helper class that indicates if the Compare paranetr is derived from
    ...
    //"compare-to" comparison taht returns a negative value to indicate
    //less-than, zero to indicate equality and a positive value 
    //to indicate greater-than. This helper class is specialized for
    ...
    //efficient compare-to code when using common google sting types
    ...
    struct btree_key_compare_to_adapter<std::greater<std::string>> : public btree_key_compare_to_tag {
        ...
        int operatot()(const std::string& a, const std::string& b) {
            return b.compare(a);
        }
    };
    ...
    struct btree_key_comparer {
        ...
        static bool_compare(const Compare& comp, const Key& x, const Key& y) {
            return comp(x, y);
        }
        bool operatot()(const Key& x, const Key& y) const {
            return bool_compare(comp, x, y);
        }
        Compare comp;
    };

    //A specialization of btree_key_compare when a compare-to funstor is
    ...
    struct btree_common_params {
        //IF Compare is derived from btree_key_compare_to_tag then
        //use it as the key_compare type. Otherwise, use btree_key_compare_to_adapter<>
        //which will fall-back to Compare if we don't have an appropriate specialization.
        typedef std::conditional_t<btree_is_key_compare_to<Compare>,
            Compare,
            btree_key_compare_to_adapter<Compare>> key_compare;
        //A type indicates is we have key-compare-funcor or we use a plain
        //key-compare functor
        ...
        enum {
            ...
            //Available space for values. This os largest for leaf nodes,
            //which has overhead no fewer than two pointers
            kNodeValueSpace = TargetNodeSize - 2 * sizeof(void*),
        };

        //This is an intengeral type large enough to hold as many
        //ValueSize-values as will fit a node of TragetNodeSize bytes.
        typedef std::conditional_t<(kNodeValueSpace / ValueSpace) >= 256,
                uint16_t,
                uint8_t> node_count_type;
    };

    //A parametrs structure for holding the type parametrs for a map.
    ...
    struct btree_set_params : public btree_common_params<Key, Compare, Alloc, TargetNodeSize, sizeof(Key)> {
        typedef std::fales_type data_type;
    ...
    struct btree_binary_search_compare_to {
        ...
        static int upper_bound(const K& k, const N& n, CompareTo comp) {
            typedef btree_upper_bound_adapter<K,
                    btree_key_compare<K, CompareTo, true>> upper_compare;
            return n.linear_search_plain_compare(k, 0, n.count(), upper_compare(comp));
        }
    };


    //A node in the btree holding. The same node type is used for 
    //both internal and leaf nodes in the tree, though the nodes are
    //allocated in such a way that the children array is only 
    //valid in internal nodes.
    ...
    public:
        ...
        //Typedefs for the various types of node search
        ...
        typedef std::conditional_t<Pramas::is_key_compare_to::value,
        ...
        struct base_fields {
            ...
            field_type position;
            //The  maximum numbers of values the node can hold.
            field_type count;
        ...
        enum {
            ...
            //We need a minimum of 3 values per internal node in order to perfom
            //spliting (1 value for the two nodes involved in the split and 1 value
        ...
        struct internal_fields : public leaf_fields {
            //The array of child pointer. The keys in children_[i] are all less than 
    ...
    public:
        ...
        //Getter for the key/value at position i in the node.
        ...
        //Move value i in this node to value i in x node/
        ...
        //Move value i to j in the node.
        void move_value(int i, int j) {
            assert(i != j);
            construct_value(i, std::move(fields_.values[j]));
            destroy_value(i);
        }

        //Gtter/setter for the child at position i in the node
        ...
        void swap_child(int i, btree_node* x, int j) {
            ...
            btree_swap_helper(a, b);
            a->fields_.parnet = this;
            a->fields_.position = i;
            b->fileds_.parent = x;
            b->fields_.position = j;
        }

        //Swap child i in this node with child i in x node.
        ...
        void swap_child(int i, int j) {
            assert(i != j);
            assert(!is_leaf());
            assert(fileds_.children[i] != nullptr);
        ...
        //Move child i in this node to child j in x node.
        ...
        //Move child i in this node to child i in x node.
        ...
        //Returns the position of the first value whouse key is greater than k.
        template <typename Compare>
        int upper_bound(const key_type& k, const Compare& comp) const {
            return search_type::upper_bound(k, *this, comp);
        }

        //Returns the position of the first value whouse value is not less than k
        //using linear search perfomed using plain compare
        template <typename Compare>
        ...
        //Returns the position of the first value key whose is not less than k
        //using linear search using compare-to.
        template <typename CompareTo>
        int linear_search_compare_to(const key_type& k, int s, int e, const CompareTo& comp) const {
            for ( ; s < e; ++s) {
                int c = comp(key(s), k);
                if (c == 0) {
        ...
        template <typename Compare>
        int binary_search_plain_compare(const key_type& k, int s, int e, const Compare& comp) {
            while (s != e) {
                int mid = (s + e) / 2;
        ...
        //Returns the position of the first value whose key is not less than k
        //using binary search perfoming using compare-to.
        template <typename CompareTo>
        int binary_search_compare_to(const key_type& k, int s, int e, const Compare& comp) {
            while (s != e) {
                int mid = (s + e) / 2;
        ...
        //and children at position >= i to the right by 1.
        void insert_value(int i, Args&&... args);

        //Removes the value at position i, shiftiing all existing values
        //and children at position > i to the left by 1.
        void remove_value(int i);

        //Rebalance a node with its right sibling.
    ...
    private:
        ...
        void construct_value(value_type* v, Args&&... args) {
            ...
            //FIXME: The above shuold use allocator_trait, but allocator
        ...
        void destroy_value(value_type* v) {
            ...
            assert(memcmp(zero_value, v, sizeof(value_type)));
        }
    ...
    private:
        btre_node(const btree_node&);
        //the same thing as btree_node(const btree_node&) = delete;
        void operator=(const btree_node);
        //the same thing as operator=(const btree_node) = delete;
    };
    ...
    private:
            ...
            typename <template> friend class btree;
            friend iterator;
            friend const_iterator;
    };

    //Dispathc helper class for using btree::internal_locate with compare-to.
    ...
    //Dispatch helper class for using btre__internal_locate with plain compare.
    ...
    class btree : public Params::key_compare {
        ...
        typedef typename btree_node::base_fields base_fields;
        typedef typename btree_node::leaf_fields leaf_fields;
        typedef typename btree_node::internal_fields internal_fields;
        typedef typename btree_node::root_fields root_fields;
        ...
        typedef std::consitional_t<is_key_compare_to::value,
        ...
        enum {
                ...
                kMatchMask = node_type::kMatch,ask,
        };
        ...
        public:
                ...
                typedef typename Params::pointer pointer;
                typedef typename Params::const_pointer const_pointer;
                typedef typename Params::refrence reference;
                typedef typename Params::const_reference const_reference;
                typedef typename Params::size_type size_type;
                typedef typename Params::difference_type difference_type;
        ...
        public:
                ...
                //Insert a value into the btree only if it does not already exist. The
                //boolean return value indicates whether insertion succeeded or failed.
                template <typename... Args>
                std::pair<iterator, bool> emplace_unique_key_args(const key_type& key, Args&&... args);

                template <typename P>
                std::pair<iteartor, bool> emplace_unique(P&& x) {
                ...
                template <typename... Args>
                std::pair<iteartor, bool> emplace_unique(Args&&... args) {
                ...
                template <class P>
                std::pair<iterator, bool>
                emplace_unique_extract_key(P&& x, btree_exctract_key_first_tag) {
                ...
                iterator emplace_hint_unique(const_iterator hint, Args&&... args) {
                    value_type v(std::forward<Args>(args)...);
                    retuen emplace_hint_unique_key_args(hint, params_type::get_key(v), std::move(v));
                }

                template <class P>
                iterator
                emplace_hint_unique_extract_key(const_iterator hhint, P&& x, btree_extract_fail_tag) {
                    value_type v(std::forward<P>(x));
                    return emplace_hint_unique_key_args(hint, params_type::get_key(v), std::move(v));
                }
                ...
                template <class P>
                iterator
                emplace_hint_unique_extract_key(const_iterator hint, P&& x, btree_ectract_key_first_tag) {
                ...
                iterator insert_unique(const_iterator hint, value_type&& v) {
                    return empalce_hint_unique_key_args(hint, params_type::get_key(v), std::move(v));
                }
                ...
                template <typename P, typename = typename std::enable_if<
                    !std::is_same<
                            std::remove_const<std::remove_reference<P>::type>::type,
                            value_type
                    >::value>::value>
                ...
                template <typename... Args>
                iterator emplace_multy_key_args(const key_type& key, Args&&... args);

                //Insert a value into the btree.
                template <typename... Args>
                iterator emplace_multy(Args&&... args) {
                    value_type v(std::forward<Args>(args)...);
                    return emplace_multy_key_args(params_type::get_key(v), std::move(v));
                }

                //Insert with hint. Chevk to see if the value should be placed immediately
                //before hint position in the tree. If it does, then the insertion will take
                //amortized constant time. If not, the insertion will take aromtized
                ...
                //the one that was erased (or end() if node exists).
                iterator erase(const_iterator iter);

                //Erase range. Returns the number of keys erased.
                int erase(const_iterator begin, const_iterator end);

                //Erase the specified key from the btree. Returns 1 if an element was
                //erased and 0 otherwise.
                int erase_unique(const key_type& key);

                //Erase all of the entires matcging the specified key from the
                ...
                size_type count_multi(const key_type& key) const {
                    return distance(lower_bound(), upper_bound(key));
                }
                ...
                //Verifies the structur of the btree.
                void verify() const;
                ...
                size_type internal_nodes() const {
                    return internal_stats(root()).interanal_nodes;
                }
                ...
                static double average_bytes_per_vale() {
                    // 1523 in gitHub. Read about it.
                    return sizeof(leaf_fields) / (kNodeValues * 0.75);
                }

                //The fulness of the btree. Computed as the number of elements in the btree
                //divided by the maximum number of elements a tree with the current number
                //of nodes could hold. A value of 1 indicates perfect space 
                //utilization. Smaller values indicates space wastage.
                double fullness() const {
                    return double(size()) / (nodes() * kNodeValues);
                }
                //The overhead of the btree structire in bytes per node. Computed as the
                //total number of bytes used by the btree minus the number of bytes used 
                // for storing elements divided by number of elements.
        ...
        private:
                ...
                //Right most is stored in the root node.
                node_type* rightmost() {
                ...
                void rebalance_or_plit(iterator& iter);

                //Merges the values of left, right and the delimiting key ont their parent
                //onto left, removing the delimiting key and deleting right.
                void merge_nodes(node_type* left, node_type* right);

                //Tries to merge node with its left or right sibling, and failing that,
                //rebalance with its left or right sibling. Returns true if a merge
                //occured, at which point it is no longer valid to access node. Returns
                //false is no merging took place.
                ...
                //Insert a value into the btree immediatly before iter. Rquires that
                ...
                //pointing at. "iter" might be pointing to an invalid location as
                ...
                node_stats internal_stats(const node_type* node) const {
                    ...
                    for (int i = 0; i <= node->count(); ++i) {
                        res += internas_stats(node->child(i));
                    }
                    return res;
                }
        ...
        private:
                //A never instintiated helper function that return big_ if we have a
                //key_compare-to function or if R is bool and small_ otherwise
                template <typename R>
    ...
    inline void btree_node<P>::insert_value(int i, Args&&... args) {
        ...
        //Initialaze new value at the end
        construct_value(cnt, std::forward<Args>(args)...);

        //Move intialized value to the correct position.
        if (cnt > i) {
            if (is_leaf()) {
                for (int j = cnt; j > i; --j) {
                    swap_value(j - 1, j);
    ...
    void btree_node<P>::rebalance_right_to_left(btree_node* src, int to_move) {
        ...
        //Move the delimiting value from the right node.
        src->move_value(to_move - 1, get_parent(), position());

        if (is-leaf()) {
            //Move the values from the right to the left node.
            for (int i = 1; i < to_move; ++i) {
                src->move_value(i - 1, this, cnt, cnt + i);
            }
    ...
    void btree_node<P>::split(btree_node* dst, int insert_position) {
        ...
        } else if (inssert_position != max_count()) {
            dst_cnt = cnt / 2;
    ...
    void btree_node<P>::merge(btree_node* src) {
        ...
        assert(position() + 1 == src->posotion());
        ...
        if (is_leaf()) {
            ...
            //Move the value and child pointer from right to the left node.
            for (int i = 0; i < src_cnt; ++i) {
                src->move_value(i, this, 1 + cnt + i);
                src->move_child(i, this, 1 + cnt - i);
            }
    ...
    btree<P>::emplace_unique_key_args(const key_type& key, Args&&... args) {
        ...
        if (res.second == kExactMatch) {
            ...
            if (last.node && !compare_keys(key, last.key())) {
                //The key already exist in the tree, do nothing.
    ...
    template <typename P> twmplate <typename... Args>
    ...
    btree<P>::emplace_hint_unique_key_args(const_iterator hint, const key_type& key, Args&&... args) {
        if (!empty()) {
            ...
            } else fi (compare_keys(hint.key(), key)) {
    ...
    btree<P>::emplace_hint_multi_key_args(const_iterator hint, const key_type& key, Args&&... args) {
        ...
        return empalce_multi(std::forward<Args>(args)...);
    }

    template <typename P>
    void btree<P>::assign(const slef_type& x) {
        clear();
    ...
    typename btree<P>::iterator btree<P>::erase(const_iterator iter) {
        ...
        if (!iter.node->is_leaf()) {
            ...
            //value of our left child. This is easy, we just decrement left.
        ...
        //Adjust our return value. If we're pointiong at the end of a node, advance
        //the iterator.
    ...
    int btree<P>::erase_multi(const key_type& key) {
        ...
        iterator end = internal_upper_bound(key, iterator(root(), 0));
    ...
    void btree<P>::try_shrink() {
        ...
        if (root()->is_leaf()) {
            ...
            if (child->is_leaf()) {
                //The child is leaf node so simply make it the root node in the tree.
                child->make_root();
                delete_internal-root_node();
                __root() = child;
            } else {
    ...
    inline IterType btree<P>::internal_last(IterType) {
    ...
    btree<P>::internal_emplace(const_iterator hint, Args&&... args) {
        ...
        if (!iter.node->is_leaf()) {
            ...
            //previous value which is guaranted to be on a leaf node.
            --iter;
            ++iter.position;
        }
        if (iter.node->count() == iter.node->max-count()) {
            //Make room in the leaf for the new item.
            if (iter.node->max_count() < kNodeValues) {
                //Insertion into the root is smaller than the full node.
                //size. Simply grow the size of root node.
    ...
    template <typename P> template <typename IterType>
    IterType btree<P>::internal_lower_bound(const key_type& key, IterType) const {
        if (iter.node) {
            for (;;) {
                iter.position = 
    ...
    template <typename P> template <typename IterType>
    IterType btree<P>::internal_upper_bound(const key_type& key, IterType) const {
        if (iter.node) {
            for (;;) {
    ...
    IterType btree<P>::internal_find_unique(const key_type& key, IterType iter) const {
        if (iter.node) {
            ...
            if (!res.second) {
                ...
                if (iter.node && !compare(key, iter.key())); {
                    return iter;
                }
            }
        }
    ...
    public:
            ...
            typedef typename Tree::reverse_iterator reverese_iterator;
    ...
    public:
            ...
            const_reverse_iteartor rbegin() const {
                return __tree.rbegin();
            }
            const_reverse_iterator crbegin() const {
            ...
            size_type byte_used() const {
                return __tree.byte_used();
            }
    ...
    public:
            ...
            btree_unique_container(const key_compare& comp = key_compare(),
                                            const allocator_type& alloc = allocatortype())
            ...
            const_iterator fin(const key_type& key) const {
                return this->__tree.find_unique(key);
            }
            ...
            std::pair<iterator, bool> insert(const value_type& v) {
                return this->__tree.insert_unique(v);
            }
            template <typename P>
            std::pair<iterator, bool> insert(P&& v) {
                return this->__tree.insert_unique(std::forward<P>(v));
            }
            std::pair<iterator, bool> insert(value_type&& v) {
                return this->__tree.insert_unique(std::move(v));
            }
            iterator insert(const_iterator hint, const value_type& v) {
                return this->__tree.insert_unique(hint, v);
            }
            template <typename P>
            std::pair<iterator, bool> insert(const_iterator hint, P&& v) {
                return this->__tree.insert_unique(hint, std::forward<P>(v));
            }
            iterator insert(const_iterator hint, value_type&& v) {
                return this->__root.insert_unique(hint, std::move(v));
            }
            void insert(std::inzializer_list<value_type> il) {
            ...
            iterator emplace_hint(const_iterator hint, Args&&... args) {
                return this->__tree.emplace_unique(hint, std::forward<Args>(args)...);
            }
            ...
            //Erase the specified iterator from the btree. The iterator must be valid
            // (i. e. not equal to end()). Return an iterator pointing to the node after
            // the one that was erased (or end() if not exists).
    ...
    public:
            ...
            // Insertion routines.
            std::pair<iterator, bool> insert(const value_type& v) {
                return this->__tree.insert_multi(v);
            }
            template <typename P>
            std::pair<iterator, bool> insert(P&& v) {
                return this->__tree.insert_multi(std::forward<P>(v));
            }
            std::pair<iterator, bool> insert(value_type&& v) {
                return this->__tree.insert_multi(std::move(v));
            }
            iterator insert(const_iterator hint, const value_type& v) {
                return this->__tree.insert_multi(hint, v);
            }
            template <typename P>
            std::pair<iterator, bool> insert(const_iterator hint, P&& v) {
                return this->__tree.insert_multi(hint, std::forward<P>(v));
            }
            iterator insert(const_iterator hint, value_type&& v) {
                return this->__root.insert_multi(hint, std::move(v));
            }
            void insert(std::inzializer_list<value_type> il) {
                insert(il.begin(), il.end());
            }
            ...
            template <typename... Args>
            iterator emplace_hint(const_iterator hint, Args&&... args) {
                return this->__tree.emplace_multi(hint, std::forward<Args>(args)...);
            }
